...
void main()
{
  ...
  servidor.sin_addr.s_addr = INADDR_ANY; //acepta servidore de todo tipo, no solo de interfaz privada, puede recibir de tipo pública o host
  servidor.sin_port = htons((u_short) PORT);
  memset(&(servidor.sin_zero), '\0', 8);

  printf( "3. bind(fd_s, (struct sockaddr *) &servidor, sizeof(servidor));\n\n" );
  bind(fd_s, (struct sockaddr *) &servidor, sizeof(servidor)); //el socket se liga a una IP

  printf( "4. listen(fd_s, QLEN);\n\n" );
  listen(fd_s, QLEN); //se le pasa el número de conexiones a encolar
  ...
  while( visitas < 5)
  {
    ...
    fd_c = accept(fd_s, (struct sockaddr *) &cliente, &longClient); //si llega aquí es porque un cliente llega, se obtiene el fd
